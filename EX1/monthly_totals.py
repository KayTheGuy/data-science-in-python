...
def pivot_months_pandas(data):
    ...
    # pivot to make <name> the new rows and <month> the new columns with <precipitation> as values
    monthly = totals_grouped.pivot(index ='name', columns='month', values='precipitation')


    #===============================
    # make precipitation counts data
    #===============================

    counts_grouped = data.groupby(['name', 'month']).aggregate('count').reset_index()
    counts = counts_grouped.pivot(index ='name', columns='month', values='precipitation')
...
def main():
    ...
    # create csv and npz files with loop
